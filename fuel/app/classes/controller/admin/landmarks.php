...
class Controller_Admin_Landmarks extends Controller_Admin 
{
	...
	public function action_index()
	{
		...
		$landmark = Model_Landmark::find('all', array('related' => array('user', 'comments', 'voteitems','landmarkphotos')));
	...
	{
		$landmark = Model_Landmark::find_by_slug($slug, array('related' => array('user', 'comments', 'voteitems','landmarkphotos')));
	   ...
	   if (Input::post('votes'))
		{
				...
				if(@$_POST['submit']){
					...
					$update_landmark = DB::query("UPDATE `landmarks` SET `updated_at` = '$curr_date' WHERE `id` = '$voteitem->landmark_id'")->execute();
